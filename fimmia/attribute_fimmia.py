...
def attribute_dataset(model, dataset, add_noise=False):
    def attribute(
        ...
        emb_inputs,
        label,
        modality_emb_inputs=None,
        **attribution_kwargs,
    ):
        if modality_emb_inputs is not None:
            inputs = (
                loss_input.to(device),
                emb_inputs.to(device),
                modality_emb_inputs.to(device),
            )
            baselines = (
                torch.zeros_like(loss_input, dtype=torch.float32, device=device),
                torch.zeros_like(emb_inputs, dtype=torch.float32, device=device),
                torch.zeros_like(
                    modality_emb_inputs, dtype=torch.float32, device=device
                ),
            )
        else:
    ...
    for item in tqdm(dataset):
        ...
        modality_embedding_input = (
            item.get("image_embedding_input")
            or item.get("video_embedding_input")
            or item.get("audio_embedding_input")
        )
        ...
        attrs_loss, *attrs_embs = attribute(
            attribution,
            loss_input,
            embedding_input,
            label=item["label"].item(),
            modality_emb_inputs=modality_embedding_input,
            **attribution_kwargs,
        )
        ...
        attrs_modality_list.append(attrs_modality[0].cpu().numpy()) if 0 != len(
            attrs_modality
        ) else None
    ...
    attrs_modality_list = (
        np.vstack(attrs_modality_list) if 0 != len(attrs_modality_list) else None
    )
...
def main():
    ...
    model_dir = (
        model_dir.joinpath(args.dataset_name) if args.dataset_name is not None else None
    )
    ...
    model_args.modality_embedding_size = args.modality_embedding_size
    model_args.embedding_size = args.embedding_size

    model = init_model(model_args)

    ds_test = get_streaming_ds(
        args.mds_dataset_path,
    )

    attrs_loss_list, attrs_embs_list, attrs_modality_list, labels_list = (
        attribute_dataset(model, ds_test, add_noise=args.add_attribution_noise)
    )
    ...
    attrs_modal_cheb_norm = (
        np.linalg.norm(attrs_modality_list, ord=np.inf, axis=1)
        if args.separate_modality_embeddings
        else None
    )
    attrs_modal_l2_norm = (
        np.linalg.norm(attrs_modality_list, ord=2, axis=1)
        if args.separate_modality_embeddings
        else None
    )
