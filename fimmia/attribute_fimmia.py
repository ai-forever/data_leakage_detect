...
def attribute_dataset(model, dataset, add_noise=False):
    ...
    for item in tqdm(dataset):
        ...
        attrs_loss, *attrs_embs = attribute(attribution, loss_input, embedding_input, label=item["label"].item(),
                                            modality_emb_inputs=modality_embedding_input, **attribution_kwargs)
...
def main():
    ...
    attrs_modal_cheb_norm = np.linalg.norm(attrs_modality_list, ord=np.inf,
                                           axis=1) if args.separate_modality_embeddings else None
    attrs_modal_l2_norm = np.linalg.norm(attrs_modality_list, ord=2,
                                         axis=1) if args.separate_modality_embeddings else None

    if args.separate_modality_embeddings:
        data = (
        attrs_loss_list, attrs_cheb_norm, attrs_l2_norm, attrs_modal_cheb_norm, attrs_modal_l2_norm, labels_list)
