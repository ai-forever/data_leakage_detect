...
def main():
    ...
    model_args = ModelArguments(model_name=args.model_name, model_path=args.model_path)
    model = init_model(model_args)
    training_args = TrainingArguments(
        **asdict(
            DefaultTrainingArguments(output_dir=args.model_path, eval_strategy="no")
        ),
        remove_unused_columns=False,
    )
    data_collator = create_data_collator(
        model_name=args.model_name,
        sigmas_path=args.sigmas_path,
        sigmas_type=args.sigmas_type,
    )
    trainer = Trainer(
        model=model,
        args=training_args,
