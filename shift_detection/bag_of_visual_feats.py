...
def extract_sift_features(images, max_features=50):
    ...
    all_descriptors = np.vstack(
        [descriptor for descriptor in descriptors_list if descriptor is not None]
    )
...
def extract_all_features(
    images, sift_clusters=50, dct_coeffs=20, color_bins=32, lbp_points=8
):
    ...
    features = np.hstack([sift_features, dct_features, color_features, lbp_features])
    return features
...
def evaluate_image_classifier(
    ...
    for train_idx, test_idx in splits:
        ...
        y_train, y_test = (
            operator.itemgetter(*train_idx)(y),
            operator.itemgetter(*test_idx)(y),
        )
...
def get_model(model_type):
    ...
    if model_type == "random":
        ...
        return GradientBoostingClassifier(
            n_estimators=50, learning_rate=1.0, max_depth=1, random_state=42
        )
        ...
        estimators = [
            ...
            (
                "gradient",
                GradientBoostingClassifier(
                    n_estimators=50, learning_rate=1.0, max_depth=5, random_state=42
                ),
            ),
        ]
...
def evaluate_random_labels(
    images, y, params, fpr_budget, n_splits=5, random_state=42, reduce_dim=True
):
...
def bag_of_visual_words_basic(
    X, y, dataset_name, fpr_budget, plot_roc=False, hypersearch=False
):
    ...
    if hypersearch:
        best_params, best_auc = hyperparam_search(
            images, y, param_grid, fpr_budget=fpr_budget, n_splits=5
        )
    ...
    auc_scores, tpr_scores = evaluate_image_classifier(
        images, y, best_params, fpr_budget=fpr_budget, n_splits=5, reduce_dim=True
    )
    print(f"Original Task (AUC): {mean(auc_scores):.4f} ± {pstdev(auc_scores):.4f}")
    print(
        f"Original Task (TPR@{fpr_budget}%FPR): {mean(tpr_scores):.4f} ± {pstdev(tpr_scores):.4f}"
    )

    # Evaluate on random-split task (negative class only)
    rand_auc, rand_tpr = evaluate_random_labels(
        images, y, best_params, fpr_budget=fpr_budget, n_splits=5, reduce_dim=True
    )
    print(f"Random-Split Task (AUC): {mean(rand_auc):.4f} ± {pstdev(rand_auc):.4f}")
    print(
        f"Random-Split Task (TPR@{fpr_budget}%FPR): {mean(rand_tpr):.4f} ± {pstdev(rand_tpr):.4f}"
    )
