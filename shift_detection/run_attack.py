import argparse
import numpy as np
from shift_detection.date_detection import date_detection_arxiv, date_detection_basic
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Blind Attacks on Membership Inference Attack Evaluation Datasets"
    )
    parser.add_argument(
        "--dataset",
        help="dataset name",
        choices=[
            ...
            "vl_mia_text_64",
            "vl_mia_img_Flickr",
            "vl_mia_img_Flickr_2k",
            "vl_mia_img_Flickr_10k",
            "vl_mia_img_dalle",
            "custom",
        ],
        default="bookmia",
    ...
    parser.add_argument(
        "--custom_feature_column", help="column name for features in custom dataset"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--plot_roc", help="set to plot FPR vs TPR curve", action="store_true"
    )
    parser.add_argument(
        "--hypersearch",
        help="set to redo hyperparam search instead of using default params",
        action="store_true",
    )
    parser.add_argument(
        "--fpr_budget", help="x for computing TPR@x%FPR", type=float, default=1
    )
    ...
    dataset_name = (
        args.dataset
        if not args.dataset.startswith("vl_mia_")
        else args.dataset.rsplit("_", 1)[0]
    )
    ...
    if args.attack == "date_detection":
        ...
        if dataset_name == "temporal_wiki":
            greedy_selection_wiki(
                members, nonmembers, dataset_name, args.fpr_budget, args.plot_roc
            )
