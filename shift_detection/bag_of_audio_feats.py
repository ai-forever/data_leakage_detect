...
def evaluate_random_labels(
    audio_paths, y, params, fpr_budget, n_splits=5, random_state=42, reduce_dim=True
):
...
def bag_of_audio_words_basic(
    audio_paths, y, dataset_name, fpr_budget, plot_roc=False, hypersearch=False
):
    ...
    if hypersearch:
        best_params, best_auc = hyperparam_search(
            audio_paths, y, param_grid, fpr_budget=fpr_budget, n_splits=5
        )
    ...
    auc_scores, tpr_scores = evaluate_audio_classifier(
        audio_paths, y, best_params, fpr_budget=fpr_budget, n_splits=5, reduce_dim=True
    )
